...
public class TransformationScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (!ObjectScript.lastDragged) return;
        ...
        if (scaleUpY && rt.localScale.y < 2f)
        {
            rt.localScale += new Vector3(0, scaleSpeed * Time.deltaTime, 0);
        }

        if (scaleDownY && rt.localScale.y > 1f)
        {
            rt.localScale -= new Vector3(0, scaleSpeed * Time.deltaTime, 0);
        }

        if (scaleUpX && rt.localScale.x < 2f)
        {
            rt.localScale += new Vector3(scaleSpeed * Time.deltaTime, 0, 0);
        }

        if (scaleDownX && rt.localScale.x > 1f)
        {
            rt.localScale -= new Vector3(scaleSpeed * Time.deltaTime, 0, 0);
        }

        isTransforming = rotateCW || rotateCCW || scaleUpY || scaleDownY || scaleUpX || scaleUpY;
    }
